...
export default function EditPlan({ plan }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm<{
        ...
        features_ar: plan.features_ar.length > 0 ? plan.features_ar : [''],
        features_en: plan.features_en.length > 0 ? plan.features_en : [''],
        limits: {
            max_rooms: plan.limits.max_rooms ?? '',
            max_images: plan.limits.max_images ?? '',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`تعديل / Edit: ${plan.name_en}`} />
            <div className="mx-auto max-w-3xl p-6">
                <h1 className="mb-6 text-2xl font-bold">تعديل باقة / Edit Plan: {plan.name_en}</h1>
